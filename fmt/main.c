...
1234 //Abhijit:the full fledged page table has been mapped on all processors.hence now reclaim all physical memory
1235 //into free list from 4+2048 MB ->224+2048 MB  VM i.e in kernel data + memory region as per kmap
1236   kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
1237   userinit();      // first user process
1238   mpmain();        // finish this processor's setup
1239 }
1240 
1241 // Other CPUs jump here from entryother.S.
1242 static void
1243 mpenter(void)
1244 {
1245   switchkvm();
